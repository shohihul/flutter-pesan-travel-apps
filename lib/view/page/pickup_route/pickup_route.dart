...
class PickupRoute extends StatefulWidget {
  ...
  @override
  _PickupRouteState createState() => _PickupRouteState();
}
...
class _PickupRouteState extends State<PickupRoute> {
  ...
  _getPassengers() async {
    ...
    setState(() {
      ...
      widget.center = passengers[0].pickupPoint;
    });
  ...
  _getPolyline() async{
    for (Passenger passenger in passengers) {
      route.add(passenger.pickupPoint);
    }
  ...
  Widget _buildItem(context, i) {
    ...
    var status = '';
    if (passenger.status == 'new')
      if (i == 0)
        status = 'enable';
      else if (passengers[i-1].status == 'new')
        status = 'disable';
      else 
        status = 'enable';
    else
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      (passenger.locationNote == null) ? 'kosong' : passenger.locationNote,
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: (passenger.status == 'new')
                          ? (i == 0)
                            ? primaryColor
                            : (passengers[i-1].status == 'new')
                ...
                child: Center(
                  child: Row(
                    ...
                    children: [
                      ...
                      : Container(),
                      Text(
                        (passenger.status == 'new') ? 'Selesai Dijemput' : 'Sudah Naik',
                        style: TextStyle(
                          color: (passenger.status == 'new') ? Colors.white : primaryColor,
                          fontSize: 12.0,
  ...
  _pickupButton(int id, index) async {
    Map data = {
      'status': 'on_travel'
    };
